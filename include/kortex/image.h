...
namespace kortex {
	...
	enum ImageType { IT_U_GRAY=1,    // uchar  1-channel
					...
					IT_J_GRAY=128,   // 16-bit 1-channel - UINT16
					IT_U_PRGBA=256   // uchar  4 channel pixel-ordered
	};
...
///
	...
	public:
		...
		const uint16_t* get_u16_ptr () const { return m_data_u16; }
		float         * get_fptr    ()       { return m_data_f;   }
		uchar         * get_uptr    ()       { return m_data_u;   }
		int           * get_iptr    ()       { return m_data_i;   }
		uint16_t      * get_u16_ptr ()       { return m_data_u16; }
		...
		uint16_t   * get_channel_u16( int cid );
		const uint16_t* get_channel_u16( int cid ) const;
		...
		// 4-channel get
		void get( int x0, int y0, uchar& r, uchar& g, uchar& b, uchar& a ) const;
		...
		void  set( const int& x0, const int& y0, const uint16_t& v );
		...
		void  set( const int& x0, const int& y0, const int& hsz, const uint16_t& v );
		...
		// 4-channel set
		void  set( const int& x0, const int& y0, const uchar& r, const uchar& g, const uchar& b, const uchar& a );
		...
		const uint16_t* get_row_u16( int y0 ) const; // use for uint16_t
...
///
	string image_type_name( const ImageType& it ) {
		...
		case IT_J_GRAY  : return "IT_J_GRAY";
		case IT_U_PRGBA : return "IT_U_PRGBA";
	...
	DataType    image_precision   ( const ImageType& it ) {
		...
		case IT_J_GRAY  : return TYPE_UINT16;
		default         : switch_fatality();
		}
		return TYPE_UCHAR;
	}
	...
	ImageType image_type ( DataType precision, int n_channels, ChannelType channel_type ) {
		switch( n_channels ) {
		case 1:
			switch( precision ) {
			...
			case TYPE_UINT16: return IT_J_GRAY;
			default         : switch_fatality();
			} break;
		case 3:
			switch( precision ) {
		...
		case 4:
			...
			case TYPE_UCHAR:
				switch( channel_type ) {
					case ITC_PIXEL: return IT_U_PRGBA;
					default: switch_fatality();
				}
			default: switch_fatality();
			} 
	...
	ImageType get_image_type( int im_type ) {
		...
		case 128 : return IT_J_GRAY;
		case 256 : return IT_U_PRGBA;
		default: switch_fatality();
		}
	}
